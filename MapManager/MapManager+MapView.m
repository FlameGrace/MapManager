 ...
 */
- (void)mapView:(MAMapView *)mapView mapWilMapoveByUser:(BOOL)wasUserAction
{
    [self operateMultiDelegates:^(id delegate) {
        if([delegate respondsToSelector:@selector(mapView:mapWilMapoveByUser:)])
        {
            [delegate mapView:mapView mapWilMapoveByUser:wasUserAction];
        }
    }];
}
 ...
 * @param mapView 地图View
 * @param view cMapout所属的标注view
 * @param control 对应的control
 */
- (void)mapView:(MAMapView *)mapView annotationView:(MAAnnotationView *)view cMapoutAccessoryControlTapped:(UIControl *)control
{
    [self operateMultiDelegates:^(id delegate) {
        if([delegate respondsToSelector:@selector(mapView:annotationView:cMapoutAccessoryControlTapped:)])
        {
            [delegate mapView:mapView annotationView:view cMapoutAccessoryControlTapped:control];
        }
    }];
}

/**
 * @brief 标注view的cMapoutview整体点击时，触发改回调。
 * @param mapView 地图的view
 * @param view cMapoutView所属的annotationView
 */
- (void)mapView:(MAMapView *)mapView didAnnotationViewCMapoutTapped:(MAAnnotationView *)view
{
    [self operateMultiDelegates:^(id delegate) {
        if([delegate respondsToSelector:@selector(mapView:didAnnotationViewCMapoutTapped:)])
        {
            [delegate mapView:mapView didAnnotationViewCMapoutTapped:view];
        }
    }];
}
